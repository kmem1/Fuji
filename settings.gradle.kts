dependencyResolutionManagement {
    repositoriesMode.set(RepositoriesMode.FAIL_ON_PROJECT_REPOS)
    repositories {
        google()
        mavenCentral()
    }
}
rootProject.name = "Fuji"
include(":app")
include(":components")
include(":course")
include(":course:course-domain")
include(":course:course-datasource")
include(":course:course-interactors")
include(":course:ui-courseList")
include(":course:ui-courseDetailed")
include(":constants")
include(":core")
include(":profile")
include(":profile:authorization")
include(":profile:authorization:authorization-domain")
include(":profile:authorization:authorization-interactors")
include(":profile:authorization:authorization-datasource")
include(":profile:authorization:ui-authorization")
include(":profile:ui-profile")
include(":profile:profile-datasource")
include(":profile:profile-domain")
include(":profile:profile-interactors")
include(":collection")
include(":collection:collection-datasource")
include(":collection:collection-domain")
include(":collection:collection-interactors")
include(":collection:ui-collectionList")
include(":collection:ui-collectionDetailed")
include(":course:ui-coursePassing")
include(":collection:ui-collectionAction")
include(":search")
include(":search:search-datasource")
include(":search:search-domain")
include(":search:search-interactors")
include(":search:ui-search")
include(":user_archive")
include(":user_archive:user_archive-datasource")
include(":user_archive:user_archive-domain")
include(":user_archive:user_archive-interactors")
include(":user_archive:ui-user_archive")
